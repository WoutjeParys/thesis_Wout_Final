...
LIMITPRICE             relative value of price difference that is allowed
...
OWNELASTPEM(P,T)       compensation PEM own-elast
...
cyc_cost(Y,Z,SS)            cycling cost of short-term storage
                    ...
                    =g=
                        (S_DATA(SS,'C_E_CYCL') - S_DATA(SS,'C_E_INV'))*1000*e_cap(Y,Z,SS)
                            + sum((P,T),
                                W(P)*(S_DATA(SS,'OPEX'))*
                                p_c(Y,P,T,Z,SS)
                                *(1/2)*(S_DATA(SS,'EFF_C')/100)
                                + W(P)*(S_DATA(SS,'OPEX'))*
                                p_d(Y,P,T,Z,SS)
                                *(1/2)/(S_DATA(SS,'EFF_D')/100)
                             )
                        ;
...
#                    ;
#
## things that have to do with compensation matrix
####################################################
#
#shiftedfi(P,H,Z)..
...
#       demlimitupper
       ...
       ##########
       # include when working with moving frames
...
#       totdemandmf
       ...
       #####
       # include when analysing demand shift hour per hour
