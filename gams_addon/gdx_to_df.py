...
def gdx_to_df(gdx_file, name, type='L', domain_info=None):
    def __get_set(gdx_file, set_name):
        if sys.platform in ['linux2', 'darwin']:
            ...
            print 'ERROR', sys.platform, 'not handled'
    ...
    if sys.platform in ['linux2', 'darwin']:
        ...
        raise GamsAddOnException('ERROR', sys.platform, 'not handled')
    (out, err) = proc.communicate()
    out = out.replace('\n', '')
    content = re.search(r'/.*/', out).group(0)[1:-1].replace('\'', '').strip().split(',')

    if err:
        raise GamsAddOnException('ERROR', err)
    elif content is []:
        raise GamsAddOnException('ERROR: No content found for ', name)
    else:
        indices = []
        values = []
        print domain_info.symbols[name][0]
        if domain_info.symbols[name][0] in ['Set']:
            df[name] = False
            for data in content:
                indices.append(data.strip())
                values.append(True)
        else:
            #print content
            for data in content:
                ...
                if len(data) == 1:
                    indices.append(1)
                    values.append(__int(data[0]))
                else:
                    index = data[0]
                    if '.' in index:
                        index = tuple([__float(i) for i in index.split('.')])
                    value = __float(data[1])
